import { Request, Response } from 'express';
import { 
  CreatePlanInput, 
  UpdatePlanInput, 
  UpdatePlanModulesInput, 
  CreateSubscriptionInput, 
  UpdateSubscriptionInput,
  AVAILABLE_MODULES
} from '../types/subscription';
import { subscriptionRepository } from '../repositories/subscription.repository';

export class SubscriptionController {
  // Plan Management
  async createPlan(req: Request, res: Response) {
    try {
      const input: CreatePlanInput = req.body;
      const plan = await subscriptionRepository.createPlan(input);
      res.status(201).json(plan);
    } catch (error: any) {
      console.error('Error creating plan:', error);
      res.status(500).json({ message: 'Failed to create plan', error: error.message });
    }
  }

  async getPlans(req: Request, res: Response) {
    try {
      const activeOnly = req.query.activeOnly !== 'false'; // Default to true
      const plans = await subscriptionRepository.getAllPlans(activeOnly);
      res.json(plans);
    } catch (error: any) {
      console.error('Error fetching plans:', error);
      res.status(500).json({ message: 'Failed to fetch plans', error: error.message });
    }
  }

  async getPlan(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const plan = await subscriptionRepository.getPlanById(id);
      
      if (!plan) {
        return res.status(404).json({ message: 'Plan not found' });
      }
      
      res.json(plan);
    } catch (error: any) {
      console.error('Error fetching plan:', error);
      res.status(500).json({ message: 'Failed to fetch plan', error: error.message });
    }
  }

  async updatePlan(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const input: UpdatePlanInput = { ...req.body, id };
      
      const updatedPlan = await subscriptionRepository.updatePlan(input);
      
      if (!updatedPlan) {
        return res.status(404).json({ message: 'Plan not found' });
      }
      
      res.json(updatedPlan);
    } catch (error: any) {
      console.error('Error updating plan:', error);
      res.status(500).json({ message: 'Failed to update plan', error: error.message });
    }
  }

  async deletePlan(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const success = await subscriptionRepository.deletePlan(id);
      
      if (!success) {
        return res.status(404).json({ message: 'Plan not found' });
      }
      
      res.status(204).send();
    } catch (error: any) {
      console.error('Error deleting plan:', error);
      res.status(500).json({ message: 'Failed to delete plan', error: error.message });
    }
  }

  // Plan Modules Management
  async getPlanModules(req: Request, res: Response) {
    try {
      const { planId } = req.params;
      const modules = await subscriptionRepository.getPlanModules(planId);
      res.json(modules);
    } catch (error: any) {
      console.error('Error fetching plan modules:', error);
      res.status(500).json({ message: 'Failed to fetch plan modules', error: error.message });
    }
  }

  async updatePlanModules(req: Request, res: Response) {
    try {
      const { planId } = req.params;
      const input: UpdatePlanModulesInput = {
        plan_id: planId,
        modules: req.body.modules || []
      };
      
      // Validate module names
      const invalidModules = input.modules.filter(m => !AVAILABLE_MODULES.includes(m.name as any));
      if (invalidModules.length > 0) {
        return res.status(400).json({ 
          message: 'Invalid module names', 
          invalidModules: invalidModules.map(m => m.name) 
        });
      }

      const updatedModules = await subscriptionRepository.updatePlanModules(input);
      res.json(updatedModules);
    } catch (error: any) {
      console.error('Error updating plan modules:', error);
      res.status(500).json({ message: 'Failed to update plan modules', error: error.message });
    }
  }

  // Subscription Management
  async createSubscription(req: Request, res: Response) {
    try {
      const input: CreateSubscriptionInput = req.body;
      const subscription = await subscriptionRepository.createSubscription(input);
      res.status(201).json(subscription);
    } catch (error: any) {
      console.error('Error creating subscription:', error);
      res.status(500).json({ message: 'Failed to create subscription', error: error.message });
    }
  }

  async getRestaurantSubscription(req: Request, res: Response) {
    try {
      const { restaurantId } = req.params;
      const subscription = await subscriptionRepository.getSubscriptionByRestaurantId(restaurantId);
      
      if (!subscription) {
        return res.status(404).json({ message: 'Subscription not found for this restaurant' });
      }
      
      res.json(subscription);
    } catch (error: any) {
      console.error('Error fetching subscription:', error);
      res.status(500).json({ message: 'Failed to fetch subscription', error: error.message });
    }
  }

  async updateSubscription(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const input: UpdateSubscriptionInput = { ...req.body, id };
      
      const updatedSubscription = await subscriptionRepository.updateSubscription(input);
      
      if (!updatedSubscription) {
        return res.status(404).json({ message: 'Subscription not found' });
      }
      
      res.json(updatedSubscription);
    } catch (error: any) {
      console.error('Error updating subscription:', error);
      res.status(500).json({ message: 'Failed to update subscription', error: error.message });
    }
  }

  async cancelSubscription(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const subscription = await subscriptionRepository.cancelSubscription(id);
      
      if (!subscription) {
        return res.status(404).json({ message: 'Subscription not found' });
      }
      
      res.json({ message: 'Subscription cancelled successfully', subscription });
    } catch (error: any) {
      console.error('Error cancelling subscription:', error);
      res.status(500).json({ message: 'Failed to cancel subscription', error: error.message });
    }
  }

  // Utility Endpoints
  async getRestaurantModules(req: Request, res: Response) {
    try {
      const { restaurantId } = req.params;
      const modules = await subscriptionRepository.getRestaurantActiveModules(restaurantId);
      res.json({ modules });
    } catch (error: any) {
      console.error('Error fetching restaurant modules:', error);
      res.status(500).json({ message: 'Failed to fetch restaurant modules', error: error.message });
    }
  }

  async checkModuleAccess(req: Request, res: Response) {
    try {
      const { restaurantId, moduleName } = req.params;
      
      if (!AVAILABLE_MODULES.includes(moduleName as any)) {
        return res.status(400).json({ 
          message: 'Invalid module name',
          validModules: AVAILABLE_MODULES 
        });
      }
      
      const hasAccess = await subscriptionRepository.isModuleEnabled(restaurantId, moduleName);
      res.json({ hasAccess });
    } catch (error: any) {
      console.error('Error checking module access:', error);
      res.status(500).json({ message: 'Failed to check module access', error: error.message });
    }
  }

  // Get all available modules
  getAvailableModules(_req: Request, res: Response) {
    res.json({ modules: AVAILABLE_MODULES });
  }
}

export const subscriptionController = new SubscriptionController();
